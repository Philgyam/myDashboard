    ...
    return (
      ...
      <div className="flex items-center">
        ...
        <div className="ml-5 w-0 flex-1">
          <dl className="break-words"> {/* Allow text to break and wrap */}
            <dt className="text-sm font-medium text-gray-500">{stat.name}</dt>
            <dd className="flex items-baseline flex-wrap"> {/* Allow items to wrap */}
              <div className="text-2xl font-bold text-gray-900">{stat.value}</div>
              <div className={`ml-2 flex items-baseline text-sm font-semibold ${stat.isPositive ? 'text-green-600' : 'text-red-600'}`}>
                <span className="flex items-center"> {/* Wrap arrow and change value */}
